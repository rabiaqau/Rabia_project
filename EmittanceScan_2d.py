...
fit_pickle = sorted(glob.glob("fits_4WP/integrated/fit_integrated_trk_run_364098.pickle"))
...
#    print output,"output from initial conditions"
 ...
 #   print chi2_double_gauss_fit,"chi2_double_gauss_fit"
 ...
 #   print NDF_double_gauss_fit,"NDF_double_gauss_fit"
...
#        print luminosity_diff,"luminosity_diff"
...
#    print chi2_1d_double_gauss_fit_Vector,"chi2_1d_double_gauss_fit_Vector"
...
#        print error_on_peak,"error on peak"
    ...
    return sigma_for_luminosity, chi_NDF_double_gauss_fit, peak, chi2_double_gauss_fit, status_of_fit, staus_cov_matrix , error_peak, error_Capsigma, covariance_element, diagnol_error, parameter_value, peak_position, CapSigma_derivative_dict, numerical_CapSigma_error, numerical_peak_error,double_gauss_fit,status,Mean
...
## 2d arrays
chi2_NDF_singleGauss2d =np.array([])
peak_singleGauss2d =np.array([])
...
#            print len(exp_lumi_x),"lumi x points"
#            print len(exp_lumi_y),"lumi y points"
#            print len(exp_lumi_2d),"exp_lumi_2d"
#            print error_lumi
...
#                            graph_2d = ROOT.TGraph2D( sep , sep_x , sep_y , lumi )                        





#                            print exp_lumi_x,"lumi"

 #                           print exp_lumi_y,"y lumi"
    
                   
                            for i in range(len(sep_x)):

                                if sep_x[i]==sep_y[i]:

 
                                    average_peak = (exp_lumi_x[i] + exp_lumi_y[i])/2
                                    print exp_lumi_x[i]
                                    print exp_lumi_y[i]
                                    print sep_x[i]
                                    print sep_y[i]
                                    average_error = (err_x[i] + err_y[i])/2
                                    
#                                    print average_error
                                    Lumi_x = np.where(exp_lumi_x == exp_lumi_x[i] , average_peak, exp_lumi_x)
                                    Lumi_y = np.where(exp_lumi_y == exp_lumi_y[i] , average_peak, exp_lumi_y)

                                    Error_x = np.where(err_x == err_x[i] , average_error, err_x)
                                    Error_y = np.where(err_y == err_y[i] , average_error, err_y)



                            sep_x_2d = np.append(sep_x , np.zeros(len(sep_y) ) )
                            print sep_x_2d,"sep_x"
                            
                            
                            error_lumi = np.append(Error_x, Error_y)

#                            print error_lumi
                            sep_y_2d = np.append(np.zeros(len(sep_x) ), sep_y )
                            print sep_y_2d,"sep_y"

                            exp_lumi_2d = np.append(Lumi_x , Lumi_y)
                            print exp_lumi_2d,"lumi"



                            graph_2d = ROOT.TGraph2DErrors(len(sep_x_2d), sep_x_2d, sep_y_2d, exp_lumi_2d, np.zeros(len(sep_x_2d)),np.zeros(len(sep_y_2d)), error_lumi)



                            single_gauss_fit_2d = ROOT.TF2("single_gauss_fit_2d", single_gauss_2d_fit, -0.02, 0.02,-0.02,0.02, 5)#2d gauss (6 parameters)        




                              # peak                                                                                                                                                                      
                            single_gauss_fit_2d.SetParameter(0 , result_x[2])

                             # mean1(x)                                                                                                                                                                    

                            single_gauss_fit_2d.SetParameter(1 ,0.0)


                           # mean2(y)                                                                                                                                                                
                            single_gauss_fit_2d.SetParameter(3 , 0.0)


                           # width(x)                                                                                                                                                                    
                            single_gauss_fit_2d.SetParameter(2 , result_x[0])


                            # width(y)                                                                                                                                                                    
                            single_gauss_fit_2d.SetParameter(4 , result_y[0])
                           ...
                           # fitting                                                                                                                                                                     
                            graph_2d.Fit('single_gauss_fit_2d',"RQ")#fitting of double gaussain                                                                                                   

                            graph_2d.Draw("F")
                        

                            canvas_2d.Print('single_gauss_2d/single_gauss_2d' + '_' + str(run)+'_'+str(i_scan)+ '.png')


                             #     # NDF                                                                                                                                                                       
                            NDF=single_gauss_fit_2d.GetNDF()


                            print NDF,"NDF"

                            chi2_NDF = single_gauss_fit_2d.GetChisquare() / NDF

                            print chi2_NDF,"chi2_NDF"
                            
                            print single_gauss_fit_2d.GetChisquare(),"chi2"


 
                            peak_2d = single_gauss_fit_2d.GetMaximum()
                            

                            print peak_2d #- result_x[2],"subtration of peak"
                            print result_x[2]

                            fit_luminosity = single_gauss_2d_fit( [sep_x_2d,sep_y_2d], single_gauss_fit_2d.GetParameters())                                                                          


#                            print (fit_luminosity - exp_lumi_2d),"fit_luminosity"

                            
#                            chi = np.array([])                                                         

                            # for sep in range(len(sep_x_2d)):                               

                            #     chi2 = ( (exp_lumi_2d[sep]- fit_luminosity[sep]))**2 / (error_lumi[sep])**2 
                                
                            #     print exp_lumi_2d[sep]- fit_luminosity[sep],"sep between 2 luminosities"
                                
                            #     chi = np.append(chi, chi2)                 


                            
                            # print chi,"chi of each point"
                            # print len(chi)
                            ...
                            avg_peak = (result_x[2]+result_y[2]) / 2


#                            print result_x[2],"peak from 1d"

                            ### 2d graph

                            
#                            double_gauss_2d= SingleGauss2d(scan,run,True,len(sep_x_2d), sep_x_2d, sep_y_2d , exp_lumi_2d , error_lumi , result_x[2] ,result_x[0],result_y[0],result_x[17],result_y[17])

#                            peak_2d = double_gauss_2d[2]


#                            print peak_2d,"2d peak"

#                            print result_x[2],"peak single gauss"


#                            chi_ndf_2d = double_gauss_2d[0]
#                            print chi_ndf_2d,"chi_ndf"
                            # chi2_2d = np.append(chi2_2d,chi_ndf_2d)
                            # peakDiff = peak_2d - result_x[2]
                            # diff = np.append(diff,peakDiff)

                            # print peakDiff,"difference in peak"

                            # histogram(100,-1, 1, diff, 'peak_diff', 'plots_2d/diff_peak.png', 'peakdiff')

                            # histogram(100,0.0, 10,chi2_2d, 'chi2_ndf', 'plots_2d/chi2_ndf.png', 'chi_ndf')
                            



                            # chi2_NDF_singleGauss2d = np.append(chi2_NDF_singleGauss2d,chi_ndf_2d)

                            # peak_singleGauss2d =np.append(peak_singleGauss2d,peak_2d)


                            # singleGauss2d = {'peakSingleGauss':peak_singleGauss2d,'chi2_NDF_SingleGauss':chi2_NDF_singleGauss2d}
...
#                            print dignol_offdiagnol_derivative_of_FOM_x
                            ...
                            my_results=[num_errors,der_errors]#,singleGauss2d]
