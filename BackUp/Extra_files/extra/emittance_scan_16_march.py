...
def workingpoint(function_name, WP):
      ...
      result_x = function_name(WP, sep_x, exp_lumi_x ,err_x ,"Emittance_scan/", run, bunches, curr_0, True, early,date )
      result_y = function_name(WP, sep_y, exp_lumi_y ,err_y ,"Emittance_scan/", run, bunches, curr_1, False, early,date)
...
def analyse_scan(WP,separation, luminosity, error, path, run_number, bunches, current, Xscan,early,date):
...
#    print s1,"s1"
    ...
    graph.Fit('double_gauss_fit','')#fitting of double gaussain
...
#    mean_double_gauss =  double_gauss_fit.Mean(-5,5) ,"mean of double gauss"
   ...
   # legend.SetHeader("Double Gauss Fit")# for lagend header                    
    ...
    fileName = path + str(run_number) +"_"+str(scan_tight_mod)+"_"
    if Xscan== True:
...
fom_2d_double_gauss= np.array([])

early_2d_dgauss_peak_tc = np.array([])
late_2d_dgauss_peak_tc = np.array([])
...
early_width_x_tight_mod = np.array([])
early_width_y_tight_mod = np.array([])
...
if fit_pickle_trk:
      ...
      for filename in fit_pickle_trk:
            with open(filename,'rb') as fit_pickle_trk:
                  ...
                  for scan_tight_mod in range(len(Tight_mod)):


                        tight_mod =  workingpoint(analyse_scan, Tight_mod[scan_tight_mod])
                        ...
                        # sep_x_2d= np.append(sep_x, np.zeros(len(sep_y)))# 2d                                                        

                        # sep_y_2d= np.append( np.zeros(len(sep_x)), sep_y)#2d                                                        

                        # exp_lumi_2d =np.append(lumi_x,lumi_y)#2d                                                            

                        # graph_2d= ROOT.TGraph2D(len(sep_x_2d),sep_x_2d , sep_y_2d , exp_lumi_2d)
                        
                        # double_gauss2d = ROOT.TF2("double_gauss2d",double_gauss2D, -3, 3, -3, 3, 9)#2d gauss (5 parameters)    
                        ...
                        if tight_mod[4]== True:
                              ...
                              early_width_x_tight_mod = np.append(early_width_x_tight_mod, early_sigma_x)

                              early_width_y_tight_mod = np.append(early_width_y_tight_mod, early_sigma_y)

                              ### parameters

                        # sigma_x= tight_mod[0][0]
                              
                              
                        # sigma_y= tight_mod[1][0]

                        # peak =  tight_mod[0][2]
                        

                        # intensity= tight_mod[2]

                        # expected_lumi= expected_luminosity(sigma_x,sigma_y,intensity)

                        # double_gauss2d.SetParameter(0,peak)# amplitude,# 2d fit                                                               

                        # double_gauss2d.SetParameter(1,-0.0712)# mean1,# 2d fit                                                                
                        # double_gauss2d.SetParameter(2,sigma_x)# sigma1,# 2d fit                                                               
                        # double_gauss2d.SetParameter(3,-0.00141)# mean2,# 2d fit                                                               
                        # double_gauss2d.SetParameter(4,sigma_y)# sigma2,# 2d fit                                                               
                        # double_gauss2d.SetParameter(5,-0.00320)# mean                                                                         

                        # double_gauss2d.SetParameter(6,sigma_x)

                        # double_gauss2d.SetParameter(7,0.000564)# mean                                                                         

                        # double_gauss2d.SetParameter(8,sigma_y)


                        # graph_2d.Fit('double_gauss2d','Q')# fitting 2d fit                                                             

                        # peak_double_gauss2d =double_gauss2d.GetMaximum()
                        
                        # fom_2d = peak_double_gauss2d/ expected_lumi


                        
                        # fom_2d_double_gauss= np.append(fom_2d_double_gauss, fom_2d)
                        ...
                        if tight_mod[4]== True:
                              ...
                              early_2d_dgauss_peak_tc = np.append(early_2d_dgauss_peak_tc,peak_double_gauss2d)


                        if tight_mod[4]== False:

                              late_2d_dgauss_peak_tc = np.append(late_2d_dgauss_peak_tc,peak_double_gauss2d)
...
# peak_2d=np.concatenate((early_2d_dgauss_peak_tc,late_2d_dgauss_peak_tc))
...
# multigraph("latest/1dVs2d_double_gauss_mu_Tight_mod.png", len(mu_Tight_mod), len(mu_Tight_mod),run ,mu_Tight_mod ,run,peak_2d)




# multigraph("latest/1dVs2d_double_gauss_fom_Tight_mod.png", len(mu_Tight_mod), len(mu_Tight_mod),run ,fom_Tight_mod ,run,fom_2d_double_gauss)
